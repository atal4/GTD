...
% xgtdFilePath = 'E:\files\xgtd\test2\test2.xgtd'; % test file
% txrxFilePath = 'E:\files\xgtd\test2\test2.txrx'; % test file 
[xgtdFileName, xgtdDir] = uigetfile({'*.xgtd', 'XGTD Project (*.xgtd)'}, 'Select an XGTD Project File','E:\files\xgtd\test2\test2.xgtd');
xgtdFilePath = fullfile(xgtdDir, xgtdFileName);
txrxFileName = strrep(xgtdFileName, '.xgtd', '.txrx');
txrxFilePath = fullfile(xgtdDir, txrxFileName);
...
try
    ...
    % Parse files
    [saBlocks, saParameters, xgtdSplit] = parseFile(xgtdFilePath);
    [txrxBlocks, txrxParameters, txrxSplit] = parseFile(txrxFilePath);
    
    % Check for empty blocks
    assert(~isempty(saBlocks), 'No Study Area blocks found in file ''%s''\n', xgtdFileName);
    assert(~isempty(txrxBlocks), 'No Tx/Rx blocks found in file ''%s''\n', txrxFileName);
    ...
    assert(~isempty(m), 'No manual Study Area block found\n');
    assert(~isempty(t), 'No Transmitter block found\n');
    assert(~isempty(r), 'No Receiver block found\n');
    ...
    % Set first transmitter and receiver sets & first manual SA block status to 'active'
    ...
    for i = 1:steps
        ...
        viscircles([rxParametersNew.x{i}, 0], rxParametersNew.radius{i}, 'Color', 'b');
        % Plot a center point inside each circle to represent the receiver point
        plot(rxParametersNew.x{i}, 0, 'rs');
        scatter(rxParametersNew.x{i}, 0, [], 'r', 'square', 'filled');
        % Add a text annotation to the plot with the receiver name and values
        text(rxParametersNew.x{i}(1), 0, sprintf('%s (x = %.2f m, r = %.2f m)', rxParametersNew.name{i}, rxParametersNew.x{i}, rxParametersNew.radius{i}), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
        % Set the axis equal property to maintain the aspect ratio
        axis equal;
        drawnow;
...
function [blocks, parameters, split] = parseFile(filePath)
